$(function () {
    ...
    $(".count-down").click(function () {
        ...
        // Key Delete
        const key = $(`#key-${position}`).val();

        // Quantity
        const value = $(`#count-${position}`).val();

        const url = `{{ route('cart.destroy') }}`;

        if (value - 1 == 0 || value < 0) {
            const check = confirm("Are you want to remove this item?");

            if (check) {
                window.location.replace(`${url}?${key}`);
            } else {
                $(`#count-${position}`).val(1);
            }
        } else {
    ...
    $("#lang").change(function (e) {
        ...
        $("#flag-country").attr(
            "src",
            `http://127.0.0.1:8000/assets/frontend/img/icons/flag-${lang}.png`
        );
    });
    // Isotop Active
